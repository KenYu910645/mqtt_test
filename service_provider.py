...
def on_disconnect(client, userdata, rc):
    if rc != 0:
        logger.warning("[on_disconnect] Unexpected disconnection.")
    else:
...
def on_publish(mosq, obj, mid): # Call be after all of the handshake is completed 
...
def on_subscribe(client, userdata, mid, granted_qos):
    logger.info ("[on_subscribe] Subscribe AWK."+ "(Mid: "+ str(mid) + ")" )
...
def pub_with_log (topic, payload, qos, retain):
	'''
	...
	'''
	pub_rc = client.publish(topic=SERVICE_NAME+"/status", payload="moveing2current", qos=2, retain=False)
	logger.info("[publish] " + str(payload) + " to " + topic + " (Q" + str(qos) + ", R" + str(int(retain))+", M" + str(pub_rc[1]) + ")")
...
if __name__ == '__main__':
	...
	logger.info("Result of Connection (rc) : "+ str(rc))
	# keepalive
	...
	client.subscribe([(SERVICE_NAME+"/cmd", 2), (USER_NAME+"/status", 2)]) # Subcribe to multi topic.
	...
	while True:
		pub_with_log(SERVICE_NAME+"/status", "moveing2current", qos=2, retain=False)    
		time.sleep(3)