...
class MQTT_OBJ():
	def __init__(self,client_id="service_provider", broker_ip="iot.eclipse.org", port=1883, keepalive=10, clean_session=True, logger = None ):
		# -----   Member variable --------#
		self.logger = logger
	...
	def publish(self, topic, payload, qos, retain):
		...
		if self.available == "offline" and (not IS_PUB_WITHOUT_CONNECT): 
			...
			self.logger.info("[publish] " + str(payload) + " to " + topic + " (Q" + str(qos) + ", R" + str(int(retain))+", Mid: " + str(pub_rc[1]) + ")")
			if pub_rc[0] != mqtt.MQTT_ERR_SUCCESS : # Something wrong
				self.logger.error("[publish]" +  mqtt.error_string(pub_rc[0]) + " (Mid: " + str(pub_rc[1])) # pub_rc=(result,mid)
			return pub_rc
		...
		if self.available == "offline" and (not IS_PUB_WITHOUT_CONNECT): 
			...
			while pub_rc.is_published():
				if time.time() - t_start >= timeout: # Wait for all the handshake is completed 
					self.logger.warn("[publish] TIMEOUT! Wait CB for " + str(timeout) + " sec.")
					return (0 , pub_rc)
				time.sleep(0.01)
			self.logger.info("[publish] Completed  in " + str(round((time.time() - t_start)*1000,3)) + " ms" + ", (Mid: " + str(pub_rc[1]) + ")" )
			return (1 , pub_rc)
	...
	def on_connect(self,client, userdata, flags, rc):
		...
		self.logger.info("[connect_CB] "+ mqtt.connack_string(rc))
	...
	def on_disconnect(self,client, userdata, rc):
		...
		if rc != 0:
			self.logger.info("[on_disconnect] Unexpected disconnection.")
		else:
			self.logger.info("[on_disconnect] Successfully disconnect")

	# The callback for when a PUBLISH message is received from the server.
	def on_message(self,client, userdata, message):
		self.logger.info("[on_message] Received message '" + str(message.payload) + "' on topic '"
	...
	def on_publish(self, mosq, obj, mid): # Call be after all of the handshake is completed 
		self.logger.info("[publish_CB]" + "Complete." + "(Mid: "+ str(mid) + ")")
	...
	def on_subscribe(self, client, userdata, mid, granted_qos):
		self.logger.info ("[subscribe_CB] Subscribe AWK."+ "(Mid: "+ str(mid) + ")" )
	...
	def on_unsubscribe(self, client, userdata, mid):
		self.logger.info( "[unsubscribe_CB] Unsubscribe AWK"+ "(Mid: "+ str(mid) + ")" )  
